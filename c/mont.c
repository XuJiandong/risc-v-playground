...
bool check_result(uint64_t *result, uint64_t *expected, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        if (result[i] != expected[i]) {
            ret = false;
            printf(
                "The result is not same as expected: 0x%x, 0x%d at index %d\n",
...
int bench_384x(void) {
    ...
    const vec384 N = {0xb9feffffffffaaab, 0x1eabfffeb153ffff,
                      0x6730d2a0f6b0f624, 0x64774b84f38512bf,
                      0x4b1ba7b6434bacd7, 0x1a0111ea397fe69a};
...
int verify_384_inner(uint8_t index) {
    ...
    for (int i = 0; i < 6; i++) {
        if (result[i] != result2[i]) {
            printf(
                "failed, wrong result at index %d: %lld(correct) vs "
                "%lld(wrong)\n",
